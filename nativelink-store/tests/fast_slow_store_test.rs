...
// `populate_fast_store_many` should pull every requested blob into the fast
// store, mirroring per-blob `populate_fast_store`. Distinct digests are
// distinguished by size_bytes here so several can coexist in the test stores.
#[nativelink_test]
async fn populate_fast_store_many_populates_all_blobs_test() -> Result<(), Error> {
    let fast_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let slow_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    ...
    for (i, size) in [10usize, 20, 30].into_iter().enumerate() {
        ...
        let digest = DigestInfo::try_new(VALID_HASH, 1000 + i).unwrap();
        slow_store
    ...
    // None should be in the fast store yet.
    ...
    // Every blob should now be present and correct in the fast store.
...
// A blob absent from both stores must surface as `NotFound`, exactly as the
// per-blob `populate_fast_store` path does.
#[nativelink_test]
async fn populate_fast_store_many_missing_blob_is_not_found_test() -> Result<(), Error> {
    let fast_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let slow_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let fast_slow_store = FastSlowStore::new(
        &FastSlowSpec {
    ...
    let present_data = make_random_data(10);
    let present_digest = DigestInfo::try_new(VALID_HASH, 2001).unwrap();
    slow_store
        .update_oneshot(present_digest, present_data.into())
        .await?;
    let missing_digest = DigestInfo::try_new(VALID_HASH, 2002).unwrap();
...
// Blobs already resident in the fast store must be left untouched (no fetch
// from the slow store), while still-missing blobs in the same batch are
// pulled in.
#[nativelink_test]
async fn populate_fast_store_many_skips_already_fast_blobs_test() -> Result<(), Error> {
    let fast_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let slow_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let fast_slow_store = FastSlowStore::new(
        &FastSlowSpec {
    ...
    // `already_fast` lives only in the fast store - if the batch tried to
    // fetch it from the slow store it would fail.
    let already_fast_data = make_random_data(10);
    let already_fast_digest = DigestInfo::try_new(VALID_HASH, 3001).unwrap();
    fast_store
        .update_oneshot(already_fast_digest, already_fast_data.clone().into())
        .await?;

    // `slow_only` must be pulled into the fast store by the batch.
    let slow_only_data = make_random_data(20);
    let slow_only_digest = DigestInfo::try_new(VALID_HASH, 3002).unwrap();
    slow_store
...
#[nativelink_test]
async fn populate_fast_store_many_empty_is_ok_test() -> Result<(), Error> {
    let fast_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let slow_store = Store::new(MemoryStore::new(&MemorySpec::default()));
    let fast_slow_store = FastSlowStore::new(
        &FastSlowSpec {
